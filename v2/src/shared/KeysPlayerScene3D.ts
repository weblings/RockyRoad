...
export class KeysPlayerScene3D extends ChartScene3D {
    ...
    // In XR mode updateCamera() is a no-op (HMD owns the camera), so topDown has
    // no effect in XR. Default true — matches both AR overhead view and desktop default.
    topDown = true;

    rightHandColor: UIColor = fromHex('#2E71D6');
    leftHandColor:  UIColor = fromHex('#E33737');

    // Piano image calibration — only active in topDown desktop mode.
    pianoScale   = 1.0;
    pianoOffsetX = 0;
    pianoOffsetZ = 0;
    ...
    protected override updateCamera(dt: number): void {
        // In local-Z mode the now-line is always at Z=0.
        const nowZ = this.toZ(this.currentTime); // = 0
    ...
    // Only visible in topDown desktop mode. No-op in XR (threeScene is null).
    private syncPianoMesh(): void {
        const scene = this.threeScene;
        if (!scene) return; // XR mode — no owned THREE.Scene
