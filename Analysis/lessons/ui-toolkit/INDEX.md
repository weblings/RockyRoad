...
- [`assets-tooling-debugging.md`](assets-tooling-debugging.md) — SVG icons, the `.uikitml`→JSON
  ...
  `keepAspectRatio: true` default reflowing an element after its texture loads, `Svg`'s no-stroke
  fill-only rendering, an SVG needing explicit `width`/`height` (not just `viewBox`) to load via
  `Image`/`TextureLoader`.
