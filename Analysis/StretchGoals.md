...
### WebXR attempts
Render the active scene in VR using the WebXR Device API (`navigator.xr`). The Three.js renderer already supports WebXR (`renderer.xr`). Main work: replacing the fixed `FretCamera` with the XR camera rig, and moving the overlay HUD to a world-space panel.
