...
export class TextBatch {
    ...
    /** Call once per frame before any drawText calls. Hides all previously shown sprites. */
    begin(): void {
    ...
    /**
     * Show a text label at the given world-space position.
     *
     * @param imageScale  Same convention as the C# imageScale param — multiplied by
     *                    TEXT_WORLD_UNITS_PER_SCALE to get world-unit height.
     * @param rightAlign  If true, `position` is the right-centre anchor (for chord names).
     */
    drawText(
        text: string,
